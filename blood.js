const submitForm = () => {
  ...
  let sdpButtons = document.querySelectorAll("input[name=SDP]");
  let sdp = "";

  for (let i = 0; i < sdpButtons.length; i++) {
    if (sdpButtons[i].checked) {
      sdp = sdpButtons[i].value;
      break; // Exit the loop once the checked radio button is found
    }
  }

  console.log(sdp); // This will print the value of the checked radio button with name "SDP"
